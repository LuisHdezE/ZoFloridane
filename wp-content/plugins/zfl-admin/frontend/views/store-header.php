 ...
 * Variables disponibles: $localidades, $current_id, $current_name, $logo,
...
<header class="zsl-header" id="zslHeader">
    <div class="zsl-topline">
        <div class="zsl-topline-inner">
            <span>
                ...
                <?php echo $current_name ? 'Entrega en ' . esc_html( $current_name ) : 'Entregas en Cuba'; ?>
            </span>
            <span>
    ...
    <div class="zsl-main">
        <a class="zsl-logo" href="<?php echo esc_url( home_url( '/' ) ); ?>">
            <?php if ( $logo ) : ?>
        ...
        <form class="zsl-search" method="get" action="<?php echo esc_url( $search_action ); ?>" role="search">
            <input type="search" name="s" placeholder="¿Qué producto estás buscando?" aria-label="Buscar productos">
        ...
        <button type="button" class="zsl-loc-chip" id="zslLocChip" aria-haspopup="dialog" aria-expanded="false">
            ...
            <span class="zsl-loc-texts">
                <span class="zsl-loc-hello">Enviar a</span>
                <span class="zsl-loc-name"><?php echo esc_html( $current_name !== '' ? $current_name : 'Elige localidad' ); ?></span>
            </span>
        </button>

        <select id="zslCurrencySelect" class="zsl-currency-select" aria-label="Moneda">
        ...
        <div class="zsl-actions">
            <div class="zsl-theme-switch" id="zslThemeSwitch" role="group" aria-label="Tema visual">
                <button type="button" class="zsl-theme-option" data-zsl-theme="black" title="Tema base negro" aria-label="Usar tema base negro" aria-pressed="true">
                    <span class="zsl-theme-swatch zsl-theme-swatch-black" aria-hidden="true"></span>
                    <span class="zsl-theme-label">Negro</span>
                </button>
                <button type="button" class="zsl-theme-option" data-zsl-theme="green" title="Tema base verde" aria-label="Usar tema base verde" aria-pressed="false">
            ...
            <?php if ( $can_manage ) : ?>
                <a class="zsl-action" href="<?php echo esc_url( $panel_url ); ?>" title="Ir al panel de gestión">
            ...
            <a class="zsl-action zsl-cart" href="<?php echo esc_url( $cart_url ); ?>">
            ...
            <a class="zsl-action" href="<?php echo esc_url( $account_url ); ?>">
        ...
        <nav class="zsl-nav" aria-label="Categorías principales">
            <div class="zsl-nav-inner">
                <?php foreach ( $nav_categories as $nav_cat ) : ?>
                    <a href="<?php echo esc_url( get_term_link( $nav_cat ) ); ?>"><?php echo esc_html( $nav_cat->name ); ?></a>
                <?php endforeach; ?>
                <a class="zsl-nav-offers" href="<?php echo esc_url( add_query_arg( 'orderby', 'popularity', function_exists( 'wc_get_page_permalink' ) ? wc_get_page_permalink( 'shop' ) : home_url( '/' ) ) ); ?>">Ofertas</a>
            </div>
        </nav>
    <?php endif; ?>
</header>

<div class="zsl-loc-modal" id="zslLocModal" role="dialog" aria-modal="true" aria-label="Elige la localidad de entrega" hidden>
    <div class="zsl-loc-box">
        <header class="zsl-loc-head">
            <strong>¿Dónde quieres recibir tu pedido?</strong>
        ...
        <p class="zsl-loc-help">Selecciona la localidad para mostrarte disponibilidad y condiciones de entrega.</p>
